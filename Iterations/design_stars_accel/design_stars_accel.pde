...
float h2,w2,d2;//=height/2,=weight/2,=d/2
...
void draw() {
  ...
  fill(0, map(dist(accel_x, accel_y, w2, h2), 0, d2, 255, -10)); // Mapping distance of cursor from center to 255,-10. The opacity is the mapped distance.
  rect(0, 0, width, height);//Background
  fill(255);//White stars  
  newStar.render();
  //createLetter();
  
  accel_x = map(arduino.analogRead(0),300,600,0,width);
  accel_y = map(arduino.analogRead(1),300,600,0,height);
...
class star {
  ...
  void move() {
    ...
    if(!insideLetter(x,y))
    {
      x =x-map(accel_x, 0, width, -0.05*speed, 0.05*speed)*(w2-x); 
      y =y-map(accel_y, 0, height, -0.05*speed, 0.05*speed)*(h2-y);  
    }
    else
    {
