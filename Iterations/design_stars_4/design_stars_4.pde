...
void draw() {
  ...
  for (int i = 0; i<starArray.size(); i++) {
    ...
    boolean inLetterTemp = inLetter(s.x,s.y);
    s.glow = false;

    if (s.x<0||s.x>width||s.y<0||s.y>height) starArray.remove(i); //remove the star if it is out of the frame

    if(mousePressed)
    {
      if(s.inLetter) //Already inside letters, so don't move
      else if(!s.inLetter && inLetterTemp)//Just got into letter, so update status and don't move
        s.inLetter=true;
      else
        s.move("normal");
    }
    else//Mouse not pressed
    {
      ...
      {
        if(inLetterTemp)
          s.glow = true; //Make the star glow
        else
          s.move("float");
      }
    }
